const c = "sitelen-layer-plugin/styles.css", i = "contains";
function u(e) {
  return [...new Set((e ?? []).filter((n) => typeof n == "string" && n.length > 0))].sort();
}
function o(e) {
  if (e.routeInclude.length === 0 && e.routeExclude.length === 0)
    return "";
  const t = JSON.stringify(e.routeInclude), n = JSON.stringify(e.routeExclude), r = JSON.stringify(e.routeMatchMode);
  return `
...
function a(e) {
  ...
  }, n = JSON.stringify(t), r = o({
    routeInclude: u(e.routeInclude),
    routeExclude: u(e.routeExclude),
    routeMatchMode: e.routeMatchMode ?? i
  });
  return r ? `import { createSitelenLayerPlugin } from 'sitelen-layer-plugin';
${r}  createSitelenLayerPlugin(${n}).init();
}
` : `import { createSitelenLayerPlugin } from 'sitelen-layer-plugin';
createSitelenLayerPlugin(${n}).init();`;
}
function s(e = {}) {
  const t = {
    autoImportStyle: e.autoImportStyle ?? !0,
    autoInit: e.autoInit ?? !1,
    container: e.container ?? "",
    initConfig: e.initConfig ?? {},
    styleHref: e.styleHref ?? c,
    scriptBody: e.scriptBody ?? "",
    injectInto: e.injectInto ?? "head",
    routeInclude: u(e.routeInclude),
    routeExclude: u(e.routeExclude),
    routeMatchMode: e.routeMatchMode ?? i
  };
    ...
    transformIndexHtml: (n) => {
      ...
      if (t.autoImportStyle && r.push({
        ...
        r.push({
          ...
          children: l,
          injectTo: t.injectInto
        });
      }
      return { html: n, tags: r };
    }
  };
}
export {
  s as default,
  s as sitelenLayerVitePlugin
};
