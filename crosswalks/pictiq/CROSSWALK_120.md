...
Sources are pinned in `crosswalk-120.json`; notices are in [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md). The Pictiq source is commit `7e9663d5a1236a881faf6a030e3258cf99e74a73`.
...
One tile: **34**. Multiple tiles: **1**. No representation: **85**. Human review queue: **106**.

## Reference table

| Toki Pona | Meaning | sitelen pona | sitelen emoji | Pictiq | Mapping | Confidence | Notes |
|---|---|---|---|---|---|---|---|
| `a` | (interjection) ah, oh, ha, eh, um, oy; (particle) [placed after something for emphasis or emotion] | ligature `a` -> `U+F1900` | ❗ | example: [`punct_exclaim`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/punct_exclaim.svg) | **CONTEXTUAL** | low | The urgency tile can carry emphasis in an utterance, but not the interjection itself. |
...
| `ala` | not, nothing, no; (particle) [negates a word or phrase]; (particle) [forms a yes-no question]; (number) zero | ligature `ala` -> `U+F1902` | ❌ | [`logic_no`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_no.svg) | **PARTIAL** | high | logic_no covers no/not, but ala also expresses nothing, zero, absence, and question formation. |
...
| `esun` | trade, barter, exchange, swap, buy, sell; market, shop, fair, bazaar, place of business | ligature `esun` -> `U+F190B` | 🛒 | [`place_shop`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/place_shop.svg) + [`money_coins`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/money_coins.svg) | **COMPOSED** | medium | Shop plus cash can communicate practical commerce, but not every form of trade or exchange. |
...
| `ike` | negative quality, e.g. bad, unpleasant, harmful, unneeded | ligature `ike` -> `U+F190D` | 👎 | example: [`logic_no`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_no.svg) | **CONTEXTUAL** | medium | logic_no can reject something in context; it does not mean a bad, harmful, or unneeded quality. |
| `ilo` | tool, implement, machine, device | ligature `ilo` -> `U+F190E` | ⚙️ | [`service_tools`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/service_tools.svg) | **PARTIAL** | high | service_tools covers tools and repair, not the wider class of devices and machines. |
...
| `kasi` | plant, vegetation; herb, leaf | ligature `kasi` -> `U+F1917` | 🌴 | [`nature_flower`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/nature_flower.svg) | **PARTIAL** | medium | A flower is one plant form; kasi also covers vegetation, herbs, and leaves. |
| `ken` | can, may, ability, permission; possibility, maybe; allow, enable; (preverb) to be able to | ligature `ken` -> `U+F1918` | 💪 | example: [`logic_yes`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_yes.svg) | **CONTEXTUAL** | medium | logic_yes can answer a can-I question affirmatively; it does not encode ability or possibility. |
...
| `kili` | fruit, vegetable, mushroom | ligature `kili` -> `U+F191A` | 🍎 | [`need_food`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/need_food.svg) | **PARTIAL** | medium | need_food can indicate an edible item or hunger, while kili is specifically produce or mushrooms. |
...
| `lape` | sleep, rest, break from an activity or work | ligature `lape` -> `U+F1922` | 😴 | example: [`place_hotel`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/place_hotel.svg) | **CONTEXTUAL** | medium | A hotel may communicate a place to sleep in travel context; it does not mean sleep or rest. |
...
| `luka` | hand, arm, tactile limb, grasping limb; to grasp, interact with, feel using touch; (number) five | ligature `luka` -> `U+F192D` | ✋ | [`qty_5`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/qty_5.svg) | **PARTIAL** | high | qty_5 covers only the numerical use; it says nothing about hand or arm. |
...
| `mani` | money, currency; thing of value e.g. gold, investment, livestock | ligature `mani` -> `U+F1932` | 💰 | [`money_coins`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/money_coins.svg) | **PARTIAL** | high | money_coins means cash/pay cash and is narrower than money, wealth, or property. |
...
| `moku` | eat, drink, consume, swallow, ingest; food, edible thing | ligature `moku` -> `U+F1936` | 🍽️ | [`need_food`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/need_food.svg) | **PARTIAL** | high | need_food expresses food or hunger; moku also acts as eat, drink, and consume. |
...
| `musi` | fun, game, entertainment, art, play, amusing, interesting, comical, silly | ligature `musi` -> `U+F193B` | 😃 | [`place_disco`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/place_disco.svg) | **PARTIAL** | medium | A disco covers one context for dance and entertainment, not fun, play, games, or art broadly. |
| `mute` | many, several, very; quantity; (number) twenty | ligature `mute` -> `U+F193C` | 👐 | [`qty_5`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/qty_5.svg) | **PARTIAL** | medium | qty_5 can pragmatically suggest many, but mute also means quantity, very, twenty, and several. |
...
| `nasa` | strange, unusual, silly, abnormal, unexpected; drunk, intoxicated | ligature `nasa` -> `U+F193E` | 🌀 | example: [`drink_beer`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/drink_beer.svg) | **CONTEXTUAL** | low | Beer may imply intoxication in a specific scene; it does not mean strange, silly, or drunk. |
...
| `noka` | foot, leg, organ of locomotion, roots | ligature `noka` -> `U+F1943` | 🦵 | [`move_feet`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/move_feet.svg) | **PARTIAL** | high | move_feet represents walking; noka also means foot, leg, and roots. |
| `o` | (particle) [marks the end of a vocative (who is being spoken to)], [marks the start of an imperative (command, wish, instruction)], should | ligature `o` -> `U+F1944` | 👋 | example: [`punct_exclaim`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/punct_exclaim.svg) | **CONTEXTUAL** | low | The urgency tile may reinforce a command, but it is not a general vocative or imperative marker. |
| `olin` | to have a strong emotional bond with, e.g. affection, appreciation, compassion, respect; platonic, romantic, or familial relationships | ligature `olin` -> `U+F1945` | 💕 | [`love_heart`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/love_heart.svg) | **PARTIAL** | high | love_heart covers affection and romance but not every platonic, familial, or respectful bond. |
| `ona` | (third-person pronoun) he, she, it, they | ligature `ona` -> `U+F1946` | 👆 | **NONE** | **NONE** | high | Pictiq has no third-person pronoun tile. |
| `open` | begin, start, open, turn on; beginning | ligature `open` -> `U+F1947` | 🔓 | [`logic_yes`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_yes.svg) | **PARTIAL** | medium | logic_yes may mean open or accepted, but not beginning, starting, or turning on generally. |
| `pakala` | damaged, broken, botched, harmed, messed up; mistake | ligature `pakala` -> `U+F1948` | 💥 | example: [`service_tools`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/service_tools.svg) | **CONTEXTUAL** | medium | A repair tile can signal that something is broken in context; it does not mean damage or mistake. |
| `pali` | work, activity; create, build, design; put effort toward, take action on | ligature `pali` -> `U+F1949` | ✊ | [`service_tools`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/service_tools.svg) | **PARTIAL** | high | service_tools overlaps with repair work but does not represent general doing or making. |
...
| `pan` | grains, starchy foods, baked goods e.g. rice, sorghum, bread, noodles, masa, porridge, injera | ligature `pan` -> `U+F194B` | 🍞 | [`need_food`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/need_food.svg) | **PARTIAL** | medium | need_food covers food/hunger broadly; pan is specifically grain and starchy food. |
...
| `pilin` | experience e.g. emotion, feeling, touch; heart (physical or emotional) | ligature `pilin` -> `U+F194E` | ❤️ | [`love_heart`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/love_heart.svg) | **PARTIAL** | medium | love_heart covers affection or romance, only a narrow subset of feeling and sensation. |
...
| `pini` | finish, stop, prevent; close, disable, turn off; ended, past; edge, end, conclusion | ligature `pini` -> `U+F1950` | 🏁 | example: [`logic_no`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_no.svg) | **CONTEXTUAL** | medium | logic_no can mean closed or stopped in context; it does not lexicalize ending or completion. |
...
| `pona` | positive quality, e.g. good, pleasant, helpful, friendly, useful, peaceful | ligature `pona` -> `U+F1954` | 👍 | example: [`logic_yes`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_yes.svg) | **CONTEXTUAL** | medium | logic_yes can signal approval or OK; it does not lexicalize positive quality or usefulness. |
...
| `seme` | (particle) [indicate a question by marking missing info in a sentence]; what, which, who | ligature `seme` -> `U+F1959` | ❓ | [`punct_question`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/punct_question.svg) | **PARTIAL** | high | punct_question marks a question but does not encode the unknown argument carried by seme. |
...
| `tawa` | motion, e.g. walking, shaking, flight, travel; (preposition) to, for, going to, from the perspective of | ligature `tawa` -> `U+F1969` | ↪️ | [`move_feet`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/move_feet.svg) | **PARTIAL** | high | move_feet covers walking or going on foot, not direction, benefit, or viewpoint. |
| `telo` | liquids e.g. water, gasoline, soda, lava, soup, oil, ink | ligature `telo` -> `U+F196A` | 💧 | [`need_water`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/need_water.svg) | **PARTIAL** | high | need_water is water/drink with practical intent; telo also covers many other liquids. |
| `tenpo` | time, event, situation, moment, period, duration | ligature `tenpo` -> `U+F196B` | ⏰ | [`time`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/time.svg) | **PARTIAL** | high | time covers time and when; tenpo also spans events, situations, periods, and duration. |
| `toki` | communicate, say, think; conversation, story; language | ligature `toki` -> `U+F196C` | 🗣️ | [`comm_phone`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/comm_phone.svg) | **PARTIAL** | medium | A phone supports calling, but toki broadly covers communication, speech, thought, story, and language. |
...
| `tu` | (number) two; separate, divide, split | ligature `tu` -> `U+F196E` | ✌️ | [`qty_2`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/qty_2.svg) | **PARTIAL** | high | qty_2 covers the number two; tu also means separating, dividing, and splitting. |
| `unpa` | sex, to have sex with | ligature `unpa` -> `U+F196F` | 🍆 | [`item_condom`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/item_condom.svg) | **PARTIAL** | medium | A condom is a protection object associated with sex, not the act or concept of sex itself. |
...
| `wan` | (number) one; singular; combine, join, mix, fuse | ligature `wan` -> `U+F1973` | 🇬🇺 | [`qty_1`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/qty_1.svg) | **PARTIAL** | high | qty_1 covers one and singular; wan also means combining, joining, or mixing. |
...
| `wawa` | power, energy, strength; confident, intense, forceful; amazing, impressive | ligature `wawa` -> `U+F1975` | ⚡ | [`power_plug`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/power_plug.svg) | **PARTIAL** | medium | power_plug covers electrical power access, only one part of energy, strength, and intensity. |
...
| `wile` | want, desire, wish, require; (preverb) want to | ligature `wile` -> `U+F1977` | 💭 | example: [`need_water`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/need_water.svg) | **CONTEXTUAL** | medium | A concrete need tile is only an example of need-intent in context; it is not a lexical mapping for wile. |
