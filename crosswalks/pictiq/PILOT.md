...
|---|---|---|---|---|---|---|
| `telo` | water; liquid; fluid; wet; wash | ligature `telo` -> `U+F196A` | 💧 | [`need_water`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/need_water.svg) | **PARTIAL** | need_water is water/drink with practical intent; telo also covers liquids and related actions. |
| `moku` | food; eat; drink; consume | ligature `moku` -> `U+F1936` | 🍽️ | [`need_food`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/need_food.svg) | **PARTIAL** | need_food expresses food or hunger; moku also acts as eat, drink, and consume. |
| `mani` | money; currency; wealth; valuable property | ligature `mani` -> `U+F1932` | 💰 | [`money_coins`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/money_coins.svg) | **PARTIAL** | money_coins means cash/pay cash and is narrower than money, wealth, or property. |
| `tomo` | indoor constructed space; building; home; room | ligature `tomo` -> `U+F196D` | 🏠 | **NONE** | **NONE** | Pictiq has specific places such as hotel and shop, but no generic building or home. |
...
| `jan` | person; human; somebody | ligature `jan` -> `U+F1911` | 👤 | **NONE** | **NONE** | The current lexicon has no generic person tile. |
| `luka` | hand; arm; tactile organ; five | ligature `luka` -> `U+F192D` | ✋ | [`qty_5`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/qty_5.svg) | **PARTIAL** | qty_5 covers only the numerical use; it says nothing about hand or arm. |
| `lukin` | look; see; examine; eye | ligature `lukin` -> `U+F192E` | 👀 | **NONE** | **NONE** | No current Pictiq tile encodes looking, seeing, or an eye. |
| `tawa` | move; go; toward; for; from the perspective of | ligature `tawa` -> `U+F1969` | ↪️ | [`move_feet`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/move_feet.svg) | **PARTIAL** | move_feet covers walking or going on foot, not direction, benefit, or viewpoint. |
| `pali` | do; work; make; build | ligature `pali` -> `U+F1949` | ✊ | [`service_tools`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/service_tools.svg) | **PARTIAL** | service_tools overlaps with repair work but does not represent general doing or making. |
| `jo` | have; carry; contain; hold | ligature `jo` -> `U+F1913` | 👜 | **NONE** | **NONE** | The current protocol has no generic possession or containment relation. |
| `wile` | want; need; must; should | ligature `wile` -> `U+F1977` | 💭 | [`need_water`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/need_water.svg) | **CONTEXTUAL** | need_water can realize wile e telo in context; it cannot represent wile by itself. |
| `pona` | good; positive; useful; simple; friendly | ligature `pona` -> `U+F1954` | 👍 | [`logic_yes`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_yes.svg) | **CONTEXTUAL** | logic_yes can signal approval or OK; it does not lexicalize goodness or simplicity. |
| `ike` | bad; negative; harmful; complex | ligature `ike` -> `U+F190D` | 👎 | [`logic_no`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_no.svg) | **CONTEXTUAL** | logic_no can reject something in context; it does not mean bad, harmful, or complex. |
| `suli` | big; important; long; adult | ligature `suli` -> `U+F1963` | 🐘 | **NONE** | **NONE** | qty_plus means more, not generic size, importance, length, or adulthood. |
| `lili` | small; little; few; young | ligature `lili` -> `U+F1928` | 🐭 | **NONE** | **NONE** | qty_minus means less, not generic smallness, youth, or a small quantity. |
| `seme` | what; which; who; unknown interrogative | ligature `seme` -> `U+F1959` | ❓ | [`punct_question`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/punct_question.svg) | **PARTIAL** | punct_question marks a question but does not encode the unknown argument carried by seme. |
| `ala` | no; not; nothing; zero | ligature `ala` -> `U+F1902` | ❌ | [`logic_no`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_no.svg) | **DIRECT** | logic_no directly covers no/not; Toki Pona also permits nothing and zero readings. |
| `ken` | can; possible; ability; permission | ligature `ken` -> `U+F1918` | 💪 | [`logic_yes`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/logic_yes.svg) | **CONTEXTUAL** | logic_yes can answer a can-I question affirmatively; it does not encode ability or possibility. |
| `pilin` | feel; emotion; sensation; heart | ligature `pilin` -> `U+F194E` | ❤️ | [`love_heart`](https://github.com/markoblogo/pictiq/blob/7e9663d5a1236a881faf6a030e3258cf99e74a73/icons/svg/love_heart.svg) | **PARTIAL** | love_heart covers affection or romance, only a narrow subset of feeling and sensation. |

## Result

DIRECT **1** | PARTIAL **9** | COMPOSED **0** | CONTEXTUAL **4** | NONE **6**
...
| `tomo` | generic building or home | **Strong Candidate** | A generic destination or shelter is useful beyond the crosswalk and is not replaced by hotel or shop. |
...
| `suli` | large or important modifier | **Possible Candidate** | Physical size can be useful, but importance and adulthood should not be folded into the same tile. |
